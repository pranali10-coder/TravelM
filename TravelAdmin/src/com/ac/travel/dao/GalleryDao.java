...
{
	public void updateGallery(GalleryBean gb, Connection con) {
		// TODO Auto-generated method stub
		try (PreparedStatement pst=con.prepareStatement("update  `tbl_gallery_master` set `gallery_name`=?, `gallery_description`=?, `gallery_img`=?  where gallery_id=? ")){
			pst.setString(1,gb.getGallery_name());
			pst.setString(2,gb.getGallery_description());
			pst.setString(3,gb.getGallery_img());
			pst.setInt(4,gb.getGallery_id());
			pst.executeUpdate();
		} catch (Exception e) {
		...
		try (PreparedStatement pst=con.prepareStatement("SELECT * FROM `tbl_gallery_master` WHERE `gallery_status`=? order by gallery_name")){
			...
			try (ResultSet rs=pst.executeQuery()){
				while (rs.next()) {
					...
					gb.setGallery_description(rs.getString("gallery_description"));
	...
	public void saveGallery(GalleryBean cb, Connection con) {
		// TODO Auto-generated method stub
		try (PreparedStatement pst=con.prepareStatement("INSERT INTO `tbl_gallery_master`(`gallery_id`, `gallery_name`, `gallery_description`,  `gallery_img`, `gallery_status`) VALUES (?,?,?,?,?)")){
			pst.setInt(1,0);
			pst.setString(2,cb.getGallery_name());
			pst.setString(3,cb.getGallery_img());
			pst.setString(4,cb.getGallery_description());
			pst.setString(5,"1");
